...
ordering_term = Group(
    expr('order_key') + Optional(COLLATE + collation_name('collate')) + Optional(ASC | DESC)('direction'))
...
tests = """\
    SELECT user_id,
             invest_id,
             order_pay_tm,
             CASE WHEN invest_ori_amount IS NULL THEN invest_amount ELSE invest_ori_amount END AS invest_amount,
             ca_id
      FROM dwd.dwd_ordr_invest_full
      WHERE dt = '1'
        AND invest_status IN ('FROZEN',
                              'FINISHED',
                              'SETTLED',
                              'CLEARED',
                              'OVERDUE',
